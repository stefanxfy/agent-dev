...
Claude Code 的会话管理由 `sessionStorage.ts`（~5100行）实现，核心设计原则：
...
> **读取策略**：Claude Code 对每个 JSONL 文件采用 head 64KB + tail 64KB 双窗口定点读取（`readSessionLite()`），不读全文件，也没有独立的索引文件。agent-dev 已实现相同策略（`read_tail()` + `read_head()`）。
...
**P0-3 去重守卫**（2026-06-18 修复）：

`_reappend_metadata()` 在写之前先扫 tail 64KB，收集已有 `(type, value)` 集合，仅 append 缺失的 entry。

```python
def _reappend_metadata(self):
    # 1. 扫 tail 64KB 收集已有元数据
    existing = set()
    for entry in self.storage.read_tail(kb=64):
        t = entry.get("type")
        if t == "custom-title":
            existing.add((t, entry.get("customTitle")))
        elif t == "tag":
            existing.add((t, entry.get("tag")))
        elif t == "agent-name":
            existing.add((t, entry.get("agentName")))
        elif t == "mode":
            existing.add((t, entry.get("mode")))
...
**效果**：
- 多次调用 `resume()` 不会产生重复 entry
- 重复 `update_mode()` 不会让 mode 条目爆炸
- JSONL 文件大小稳定，不再因元数据重写而膨胀
...
**mermaid 视角**（P2-8 修复后，含 tool_result 守卫）：

```mermaid
stateDiagram-v2
    [*] --> NEED_TITLE
    NEED_TITLE --> AI_PENDING: 消息1 (非 tool_result)
    NEED_TITLE --> AI_SET: 消息1 是 tool_result<br/>(P2-8 守卫跳过)
    AI_PENDING --> AI_SET: AI 返回成功<br/>genSeq 一致
    AI_PENDING --> NEED_TITLE: AI 失败 / 超时
    AI_SET --> AI_PENDING: 消息3 (genSeq++)
    AI_PENDING --> FINALIZED: AI 返回 (genSeq=2)
    AI_SET --> FINALIZED: 消息3 跳过<br/>(无 AI 重新生成)
    FINALIZED --> USER_SET: 用户改名
    AI_SET --> USER_SET: 用户改名
    AI_PENDING --> USER_SET: 用户改名
    NEED_TITLE --> USER_SET: 用户改名
    USER_SET --> [*]
    FINALIZED --> [*]
    note right of NEED_TITLE
      起始状态
      任何重命名都会 → USER_SET
    end note
    note right of USER_SET
      永久锁定
      永不自动生成
    end note
```
...
| 第1条 = tool_result | **P2-8 守卫**：跳过 AI 生成，状态置为 `AI_SET` 占位 | 列表（content blocks） |
| 第3条用户消息 | 异步 AI 重新生成（覆盖第1条） | 完整对话文本（`_extract_conversation_text`） |
| 第4条起 | 不触发 | — |
| 用户手动改名 | 写入 `custom-title`，永久锁定 | — |

**P2-8 修复说明**（2026-06-18）：

`add_user_message()` 调用 `_on_user_message(content)` 时，content 可能是 list（含 `tool_result` blocks，常见于 resume 后的第一条消息）。如果直接拿 list 当标题语料，会生成 `[Tool Result: ...]` 这种垃圾标题。

**守卫逻辑**（`_on_user_message`）：
...
def _on_user_message(self, text):
    ...
    if isinstance(text, list) and all(
        ...
        if self._title_state == TitleState.NEED_TITLE:
            self._title_state = TitleState.AI_SET  # 视为"已生成"占位
        return  # 不进入 AI_PENDING，不调 fire_and_forget_title

    # ... 原有逻辑 ...
```

**场景示例**：
- 用户从断点恢复 → JSONL 已有若干消息 → 第一次写入是 `tool_result` → 跳过标题
- 第2条用户消息是真实 query → 正常进入 `AI_PENDING` 流程
...
> **2026-06-18 已完成**（从待补齐清单移除）：
> - ✅ `_reappend_metadata` 去重守卫 → 见 §4.4 P0-3 修复说明
...
| 会话列表 | `utils/listSessionsImpl.ts` + `utils/sessionStoragePortable.ts` | `listSessions()`, `readSessionLite()` |
| 会话恢复 | `sessionRestore.ts` | `restoreSession()`, `resetSessionFilePointer()` |
| 压缩 | `services/compact/compact.ts` | `compactConversation()` |
