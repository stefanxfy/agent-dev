...
| UI sidebar 计数 | 已有,本次新增 toggle "强制提取" |
...
| 设计文档 | 在 `docs/memory-system-design.md` 加 §3.3.1 用户调整版 + §4.6 短对话"记住"策略 + §6.8 显式意图 toggle 决策记录 |
...
| 关键词强意图短路(违背设计 §3.3 严格性)| 不做,改用 UI toggle 显式表达 |
...
- **加**:`force_extract` toggle(强制提取,绕开门1门2,本次 turn 必走 LLM 评分)
...
| 短对话"记住"空指针 | 不存在(强制提)| **不修**(不违背设计);改用 toggle 显式触发 |
...
| **短对话"记住"不响应** | 不修(违背设计);改用 UI toggle "强制提取" 显式表达 |
...
2. 加 toggle "强制提取"
3. 更新 `docs/test_react_ui.md`
4. 更新 `docs/memory-system-design.md`(加 §3.3.1 用户调整版 + §4.6 短对话策略 + §6.8 显式意图 toggle 决策记录)
