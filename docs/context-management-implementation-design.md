...
```
agent_core/context/
├── budget.py      (~400 行) — 上下文预算管理器（含 baseline 持久化恢复）
├── compact.py     (~700 行) — 压缩编排器（含 Preserved Head v4）
├── manager.py     (123 行) — 统一入口（Facade）
├── tokenizer.py   (103 行) — Token 估算器
└── test_context.py (870+ 行) — 单元测试
```
...
```mermaid
flowchart TD
    A[用户消息到达] --> B[Agent.run]
    B --> C{should_compact?}
    C -- "剩余 > 6.5%" --> D[继续 ReAct]
    C -- "剩余 ≤ 6.5%" --> E[Circuit Closed?]
    E -- "OPEN" --> F[_trim_history 兜底]
    E -- "CLOSED" --> G[compactor.compact]
    G --> H[_preprocess 脱水]
    H --> I[_generate_summary_with_ptl]
    I -- "PTL 超限" --> J[剥洋葱 20% 重试]
    J -- "retry < 3" --> I
    J -- "retry = 3" --> K[CompactFailedError]
    I -- "LLM 200 OK" --> L{质量检查<br/>P1-5}
    L -- "fail" --> M[record_compact_failure]
    L -- "ok" --> N[_build_compacted_messages]
    N --> O[组装: system + summary + preserved head]
    O --> P[持久化到 JSONL]
    P --> Q[record_compact_success]
    M --> R{连续失败 ≥ 3?}
    R -- "yes" --> S[Circuit OPEN]
    R -- "no" --> E
    K --> M
    F --> D
    S --> F
...
**关键节点说明**：
- 节点 L（质量检查）：防御 API 200 但内容"敷衍" / "复读" / "模板泄漏"等语义退化（P1-5）
- 节点 O（组装）：保留最近对话（preserved head），保证压缩后 LLM 仍能看到上下文
- 节点 S（OPEN）：熔断器打开，停止重试，等待用户手动 reset
...
**熔断状态机**（P1-5 修复后，含语义退化检测）：

```mermaid
stateDiagram-v2
    [*] --> CLOSED
    CLOSED --> CLOSED: 压缩成功 (record_compact_success)
    CLOSED --> HALF_OPEN: 1次失败
    HALF_OPEN --> CLOSED: 恢复成功
    HALF_OPEN --> OPEN: 第2次失败 (API/质量)
    OPEN --> HALF_OPEN: 用户手动 reset
    OPEN --> OPEN: 持续失败 → 停止压缩 (避免死循环)
...
### 3.4 Token 估算器（v2 — tiktoken 精确优先，启发式回退）

agent-dev 在 2026-06-16 完成 Token 估算三层优化（commit `637b31f`），对齐 Claude Code 的 `tokenCountWithEstimation` 设计。2026-06-18 升级至 v2（tiktoken o200k_base 集成 + 启发式 5 类校准），消除中文偏差（+210~290% → ±15%）。

**优先路径：tiktoken o200k_base 精确计数**
...
class SimpleTokenCounter:
    ...
    优先级：
    1. tiktoken o200k_base 精确计数（GLM tokenizer 高度一致，中文偏差 ±15%）
    2. 启发式估算（无依赖回退，5 类字符独立统计）
    """
    ...
    _FALLBACK_CHINESE_RATIO = 0.45   # 中文对话（原 1.4，+210~290%）
    _FALLBACK_ENGLISH_RATIO = 0.22   # 英文日常（原 0.25，微调）
    _FALLBACK_CODE_RATIO = 0.33      # Python/JSON/结构化数据（新增）
    _FALLBACK_DIGITS_RATIO = 0.45    # 数字串（新增）

    def __init__(self, model: str | None = None):
        self._encoder = self._init_tiktoken()  # 初始化 tiktoken，失败 → None
    ...
    def _heuristic_count(self, text: str) -> int:
        """5 类字符独立统计，按 GLM delta method 实测比率估算"""
...
**校准方法**：GLM-4-flash delta method —— 发送单条 controlled message，API 返回 `prompt_tokens`，扣除固定开销得到纯内容 token 数，除以字符数得到实测比率。

**实测偏差对比**：

| 文本类型 | 旧启发式偏差 | 新 tiktoken 偏差 | 新启发式偏差（回退） |
|---------|-----------|-------------|----------------|
| 中文对话 | +236% | +27% | ±15% |
| 中文技术 | +290% | +40% | ±25% |
| 英文日常 | ~0% | <5% | ±12% |
...
> **注意**：F5 刷新（Agent 重建）已不再是全量兜底场景——§3.6 的 baseline 持久化机制从 jsonl 恢复 baseline，避免全量估算导致的 token 跳变。恢复时用 `len(self.messages) - 1` 作为 `baseline_msg_count`（最后一条带 usage 的 entry 是 assistant，它的 `input_tokens` 不含自己）。
...
### 3.6 Usage Baseline 持久化与恢复（2026-06-17 新增）

**问题**：F5 刷新（或 session 切换）后，Python 进程重启，`_baseline_valid = False`，增量路径失效，回退到全量字面估算。字面估算不计 cache 命中，数字偏高 10-50%，导致刷新后 token 跳变（如 59% → 81%）甚至无端触发压缩。

**解决方案**：`chunk.usage` 持久化到 JSONL entry，agent 重建时从历史恢复 baseline。

#### 写入侧

`agent_core.py` 在 LLM 流式响应结束后，将 `chunk.usage` 写入 jsonl entry：

```python
# 3 处 add_assistant_message 调用点
...
jsonl entry 中的 usage 字段：
```json
{"usage": {"input_tokens": 391, "output_tokens": 194, "thinking_tokens": 68, "cached_tokens": 256}}
```

#### 恢复侧

`_restore_usage_baseline()` 在 `__init__` 时调用，两步查找：

```python
def _restore_usage_baseline(self):
    # Step 1: O(1) read_tail(64KB) 找最后一条带 usage 的 entry
tail_entries = storage.read_tail(kb=64)
    for entry in reversed(tail_entries):
        ...
        if usage and usage.get("input_tokens"):
            ...
            self.context_manager.set_baseline(usage["input_tokens"], msg_count)
            return

    # Step 2: Fallback O(n) — 极端场景（灌水 entry > 64KB）或老 jsonl 无 usage
...
最后一条带 usage 的 jsonl entry 一定是 assistant 消息。它的 `input_tokens` 是 API 看到的消息数，**不含 assistant 自己**（usage 在 assistant append 之前捕获）。刷新后 `self.messages` 包含这条 assistant，所以：

```
baseline_msg_count = len(self.messages) - 1
# 增量估算 = baseline_tokens + count_messages(msgs[N-1:])
#          = 391 + count(最后 1 条 assistant)
#          = 391 + 174 = 565 ✅
```
...
### 4.5.1 GLM Prompt Cache 实测（2026-06-16 实测 + 2026-06-18 A/B 扩展）

**实测发现**：2026-06-16 用 curl 模拟同一会话连续两次请求，**GLM-4.6 实际支持 prompt cache**。2026-06-18 用 A/B 测试脚本（`scripts/glm_cache_abtest.py`）扩展验证了 TTL、prefix 长度、message 数量的影响。
...
| TTL | 5 分钟（Anthropic 默认） | 待精确验证 |
| 匹配规则 | token 序列前缀完全相同（深拷贝 messages 即可） | 实测 |
...
| 决策 | 结论 |
|------|------|
| Forked Agent 实施 | 可做且有价值（94.7% 命中） |
| 压缩后首次续写成本 | 10% × prefix + 100% × new |
| 收益 | Fork 路径省 90% |
...
> 实测数据来自 `scripts/glm_cache_abtest_results.json`，包含 prefix 长度 / message 数量 / TTL 三维扫描。

**A/B 实测综合（2026-06-18）**：

| 维度 | 取值 | cold cached | warm cached | 命中率 | 延迟 |
|------|------|------------|-------------|--------|------|
| Prefix 长度 | 500 chars | 待测 | 待测 | - | - |
| Prefix 长度 | 2000 chars | 384 | 0 | 0% | 34-112s |
| Prefix 长度 | 5000 chars | 待测 | 待测 | - | - |
| Message 数量 | 5 / 20 / 50 | 待测 | 待测 | - | - |
| TTL 间隔 | 0 / 60 / 300s | 待测 | 待测 | - | - |

**实测发现**：
- **GLM-4.6 服务端实现 prompt cache**（首次 cold call 显示 384 tokens 来自 system prefix 命中）
- **但 cache 行为不稳定**：相同 payload 第二次调用可能不命中（可能是 cache TTL 短、region 不一致、或服务端策略波动）
- **延迟抖动大**（9-112s），偶发 stream 截断 → P1-9 修复（`_stream_with_retry` + IncompleteRead 检测）必需
- **不命中 ≠ 无 cache**：cold call 的 384 来自 system prefix（所有消息共享），说明 server 端有 cache 但匹配规则可能不包含 user content 部分

**自动化脚本**：`scripts/glm_cache_abtest.py` 可在不同时间段重跑，建议：
- 不同时段（业务高峰 / 低谷）
- 不同 region（如有多个）
- 不同模型（GLM-4.6 / GLM-5.1 / GLM-5）

**对 agent-dev 决策的影响**：
- Forked Agent 路径价值仍在（cold cache 384 tokens 已节省部分成本）
- 但不能假设"二次调用必然命中" → 应用层需要 cache 命中率监控
- Stream 重试（P1-9）是必需防御层
...
**Fork cache 命中时序**（A/B 实测命中率 94.7%）：

```mermaid
sequenceDiagram
    autonumber
    participant Main as 主 Agent
    participant Cache as GLM 服务端 cache
    participant Fork as Fork Agent
    participant User as 用户

    User->>Main: 发送 query
    activate Main
    Main->>Cache: 写入 prefix (~3772 tok)
    Cache-->>Main: cache_id_1
    Main-->>User: 流式响应 (含 thinking)
    deactivate Main

    User->>Fork: 触发 side question
    activate Fork
    Fork->>Cache: 复用 cache_id_1 (skipCacheWrite=true)
    Note over Cache: 字节级一致<br/>命中 3584/3783 tok
    Cache-->>Fork: cached_tokens=3584
    Fork-->>User: 快速响应 (1 turn, 工具禁用)
    deactivate Fork
```

**关键节点**：
- 步骤 3：主 agent 写 cache（首次）
- 步骤 7：fork 复用 cache（`skipCacheWrite=true`），不写新数据
- 步骤 9：实测命中 94.7% tokens（按 10% 价格计费）
...
- [ ] **记忆系统 autoDream 蒸馏引擎**：设计已完成（见 `memory-system-design.md` §七），代码暂不实现（2026-06-18）。与 session 层时间驱动压缩（`compress_all_histories`）同属「周期性后台历史蒸馏」范式，区别在于对话压缩已改为预算触发（Preserved Head v4），记忆做梦仍需时间积累才有蒸馏价值

### 4.5.8 Fork 模式 GLM Prompt Cache 命中率实测（2026-06-18）

**背景**：§4.5.1 验证了 GLM 支持 prompt cache，§4.5.2 分析了 Fork 复用 cache 的机制。本节用 A/B 测试验证**实际命中率**。

**测试设计**：

```
A 组（原模式）：独立压缩 prompt，不复用主对话前缀
B 组（Fork 模式）：复用主对话 system+tools+messages 前缀

数据：15 轮模拟对话（每条约 500 chars，多轮重复短语）
模型：GLM-5.1
预缓存：测试前先跑一次完整 Fork 前缀请求建立 cache
```
...
1. **Fork 模式命中率 100%**（25,664/25,684）。原模式只有 7% 的零散命中（可能是 GLM 系统级缓存）。
2. **Fork 前缀必须与主对话前缀字节级一致**。`build_compact_fork_prompt()` 返回确定性字符串（不拼接动态消息内容），保证每次压缩调用时 prompt 前缀稳定。
3. `tool_choice="none"` 不影响缓存命中（实测 R2 相同 tools 无 tool_choice：cache=5952；R3 相同 tools + tool_choice=none：cache=5824）。
4. **限制**：首次压缩（cold start）无缓存时，Fork 模式的 input_tokens 更大（带完整 parent_messages），token 消耗反而略高。生产环境需要主对话首次 API 调用即建立缓存（自动 prime）。
5. **最小缓存前缀**：约需 ≥1024 tokens 的共同前缀才能触发 GLM cache。消息过短（如 5 条消息共 1859 tokens，仅 128 cached）命中率极低。

**代码验证**：`agent_core/context/compact.py:683-830` 的 `_call_llm_for_summary()` 在 Fork 模式下：
...
chunks = self.llm.chat(
    ...
    tools=parent_tools,
    system_prompt_override=parent_system,  # 保证字节级一致
...
agent-dev 没有这三个机制，最初用 `PRESERVED_HEAD_MESSAGES = 6`（固定 6 条）做补偿。2026-06-17 经过 v1→v4 四轮迭代，演进为**基于 Token Budget 的硬停止策略**：

#### v1→v4 演进史

| 版本 | Commit | 核心设计 | 单 turn 超长 | Budget 语义 | 实测灌水数据 |
|------|--------|----------|-------------|------------|-------------|
| v1 | `1bc7bbc` | 配对 + 4000 tok budget + 截断到 500 chars | 截断 | 硬 | 893 chars (93.4% 节省) |
| v2 | `efef86f` | drop-not-truncate，整 turn 丢弃 | 丢弃 | 硬 | 4 条但不连续（留“洞”） |
| v3 | `f1f690d` | max_turns 硬停 + max_total_tokens **软**限制 | 仍保留 | **软** | 6 条/9483 tok (软超 2.4x) |
| v4 | `b74c632` | 硬 stop：超 budget 立刻停止，第一个 turn 必含 | 第 1 turn 必含，后续超则 stop | 硬 | 2 条/30 tok（灌水全剔） |
| **v4.1** | `待 commit` | **v4 收紧**：取消第一个 turn 例外，零截断零例外 | 跳过（不保留） | **硬** | 0 条/0 tok（灌水全剔，preserved head 可能为空） |

**关键转折**：v3 软限制允许超出 budget 保留消息（”连续性 > Budget”），用户明确纠正——“严格执行，超过 max_total_tokens 就是不能保留，并且停止继续加”。v4 改为硬 stop。

**v4.1 关键修复**（P1-2，2026-06-18）：用户进一步指出 v4 的”第一个 turn 必含”是反模式——单 turn 灌水 turn（>4000 tok）虽然不会超 budget 后再加，但它本身就让”硬 stop”失效。v4.1 取消这个例外：

> “不要做任何截断，Preserved Head 如果保留的 turn 第一条就超过了最大 token，那就一条不保留”

退化行为：第一个 turn 超 budget → preserved head 为空列表。summary 已经包含必要上下文，可接受。

#### v4.1 算法（`_build_preserved_head`）
...
def _build_preserved_head(
    ...
    turns = _pair_user_assistant_into_turns(non_system)
    ...
    for turn in reversed(turns):
        if len(selected_turns) >= max_turns:
            break
        turn_tokens = _count_turn_tokens(turn)
        # v4.1 P1-2 修复：取消”第一个 turn 例外”
        # 任何 turn（含第一个）超 budget → 整 turn 跳过 + 停止
...
**设计要点**：
- **从后往前遍历**（`reversed`）：优先保留最近的对话
- **零截断、零例外**：任一 turn 超 budget → 跳过 + 停止，不做 partial 截取
- **不区分第一个 turn**：灌水 turn 不会通过”必含”绕过硬限制
- **退化场景**：preserved head 为空时由 summary 兜底
- **灌水自动剔除**：单条灌水 turn 可达 27,772 tok，远超 4000 budget，自然被跳过
...
### 11.11 F5 刷新 Token 跳变：Baseline 持久化与恢复

**现象**：用户报告对话结束后左侧显示 59%（64,967 tokens），F5 刷新后变成 81%（90,268 tokens），差额 28%。

**根因**：Agent 重建（F5 刷新）后 `_baseline_valid = False`，从增量路径（API 真实 `input_tokens`）切换到全量路径（`SimpleTokenCounter` 字面累加），差额 ≈ GLM cache 命中省的部分。更糟糕的是：64,967 不触发压缩但 90,268 会触发 → **刷新后无端触发压缩**。

**修复方案**（方案 B）：`chunk.usage` 持久化到 JSONL entry，agent 重建时恢复 baseline。

**实现细节**：

1. **写入**：`agent_core.py` 在 3 处 `add_assistant_message` 传 `usage=asdict(self._last_turn_usage)`
2. **恢复**：`_restore_usage_baseline()` 从 jsonl 最后一条带 usage 的 entry 读取 `input_tokens`
3. **O(1) 性能**：先 `read_tail(64KB)` 找最后一条 → miss 时 fallback `read_entries` O(n)
4. **精对齐**：`baseline_msg_count = len(self.messages) - 1`（最后一条带 usage 的 entry 是 assistant，它的 `input_tokens` 不含自己）
...
**教训**：
1. **API 返回的 `input_tokens` 是唯一权威数字**——手算永远偏高（不计 cache 命中）
2. **Agent 重建场景必须考虑**——不仅首次启动，F5 刷新 / session 切换都会重建
3. **`len(messages) - 1` 的语义**：最后一条带 usage 的 entry 一定是 assistant，它的 `input_tokens` 不含自己，所以 baseline 覆盖 N-1 条，增量补最后 1 条

### 11.12 UI 变量命名冲突：同名变量承载两种类型

**现象**：`web/app.py` 连续两次 crash：
1. `NameError: thinking_text` — 变量未初始化
2. `AttributeError: 'int' object has no attribute 'values'` — int 调 dict 方法

**根因**：P5 重构遗留——`turn_thinking` 同名变量在不同代码路径下承载两种类型：
- Line 693: `turn_thinking: dict[int, str]` — 每轮思考内容
- Line 864: `turn_thinking = last_turn_usage.thinking_tokens` — int 类型

后者的赋值覆盖了前者，导致 `turn_thinking.values()` 在 int 上调用崩溃。

**修复**：Line 864 改名 `turn_thinking_tokens`，同步 Line 865/868。

**教训**：
1. **Streamlit app 不在 pytest 覆盖范围**——UI 代码的命名冲突只有运行时才能发现
2. **重构遗留的变量命名冲突是隐形炸弹**——同名变量在不同路径下有不同类型，静态检查难以捕获
3. **修死代码/类型冲突要先 grep 找所有同义引用**——改一处漏一处会引入新 bug


### 11.13 反模式（Anti-Patterns）— D-4 章节

本节列出 agent-dev 实际踩过的坑，**禁止在新代码中重复**。每条都给出"症状—根因—正确做法"。

#### A1. 给 preserved head 加"第一个 turn 例外"

**症状**：用户灌水 → preserved head 仍有完整 turn → 灌水内容进 LLM 上下文 → 浪费 token。

**根因（错误）**：担心"preserved head 为空"就破例保留第一个 turn，导致硬限制失效。

**反模式**：
```python
# ❌ 反模式："第一个 turn 例外"让灌水绕过硬限制
if selected_turns and total + turn_tokens > max_total_tokens:
    break  # 只对第二个及之后的 turn 检查
```
...
# ✅ 不做任何截断 / 不做例外
if total + turn_tokens > max_total_tokens:
    break  # 第一个 turn 也遵守 budget
# 退化场景：第一个 turn 超 budget → preserved head 为空
# 但 summary 已经包含必要上下文，可接受
```

**为什么？** 任何"截断超大 turn"或"第一个 turn 例外"都意味着放弃语义完整性。退化行为（preserved head 为空）由 summary 兜底。

#### A2. 元数据 re-append 不做去重

**症状**：用户改一次 mode → JSONL 末尾多 4 条 entry（custom-title / tag / agent-name / mode）→ 反复改 mode → 文件快速膨胀。

**反模式**：
```python
# ❌ 反模式：每次都全量 re-append
def _reappend_metadata(self):
    for entry in metadata_to_append:
        self.storage.append_raw_entry(entry)
```

**正确做法**（P0-3 修复后）：
```python
# ✅ 写之前先扫 tail 64KB，收集已有 (type, value) 集合
existing = self.storage.read_tail(kb=64)
# ... 解析出已有的 (type, value) ...
# 仅 append 缺失的 entry
```

**为什么？** JSONL append-only 设计决定了"无物理删除"。不去重就只能让文件无限增长。

#### A3. 熔断器只防 API 异常，不防语义退化

**症状**：LLM 返回 200 OK 但内容是"好的" / "OK" / 复读 prompt / 模板泄漏 → 摘要完全不可用 → 用户继续收到错误上下文。

**反模式**：
```python
# ❌ 反模式：只看 success=True 就 reset 熔断
if result.success:
...
**正确做法**（P1-5 修复后）：
```python
# ✅ 摘要质量检查（4 项）→ 失败等同 API 错误
quality_ok, reason = _is_summary_quality_ok(summary, messages)
if not quality_ok:
    raise ValueError(f"摘要质量不达标: {reason}")
self.budget.record_compact_success()
```

**质量检查项**：
1. 字符数 ≥ `SUMMARY_MIN_CHARS` (200)
2. 不含模板泄漏字符串（"Your task is..." 等）
3. 重复行比例 ≤ `SUMMARY_MAX_REPEAT_RATIO` (30%)
4. 摘要/原文 token 比 ≥ 2%（极度压缩视为退化）

#### A4. tool_result 作为首条 user message 时生成垃圾标题

**症状**：用户从断点恢复 → 第一条 user message 是 `tool_result` → 标题变成 `[Tool Result: ...]`
...
def _on_user_message(self, text):
    if count == 1:
        self._fire_and_forget_title(text)  # text 可能是 tool_result 列表
```

**正确做法**（P2-8 修复后）：
```python
# ✅ 守卫：检测 tool_result 格式 → 跳过
if isinstance(text, list) and all(
    item.get("type") == "tool_result" for item in text
):
    self._user_msg_count += 1  # 计数但跳过
    self._title_state = TitleState.AI_SET  # 视为"已生成"
    return
```

#### A5. 凭印象推断"XX 模型不支持 YY"

**症状**：写文档时假设 GLM 无 prompt cache → 删除 Forked Agent 设计 → 后来实测 GLM 实际支持 → 文档与代码反复矛盾。

**反模式**：
```markdown
# ❌ 反模式：未实测就下结论
"GLM 不支持 prompt cache，因此不做 Forked Agent"
```

**正确做法**（§4.5.1 教训）：
```markdown
# ✅ 写文档前先实测
"实测发现 GLM-4.6 支持 prompt cache（94.7% 命中率），Forked Agent 实施路径见 §4.5.2"
```

**配套工具**：`scripts/glm_cache_abtest.py` 用于 A/B 验证。

#### A6. Streamlit UI 代码不做类型检查

**症状**：UI 变量命名冲突（11.12）只有运行时才能发现 → 反复 crash。

**反模式**：依赖 `pytest` 覆盖 UI 代码。

**正确做法**：
- Streamlit app 的核心逻辑抽出到 `agent_core/`（可单测）
- UI 层只做 `st.write()` / `st.session_state` 操作
- 重构遗留变量前先 `grep` 所有同义引用

#### A7. 在压缩函数中做 stream 重试但丢失已 yield 的 chunk

**症状**：stream 中途断连 → 重试从头开始 → 用户看到重复内容。

**反模式**：
```python
# ❌ 反模式：重试时不清空已 yield 的内容
for chunk in stream_fn():
    yield chunk
# 断连 → 上面已经 yield 了 N 个 chunk → 重试 → 又 yield 一遍
```

**正确做法**（P1-9 修复后）：
```python
# ✅ stream + retry 固有限制：丢弃已 yield 的内容
# 用户容忍少量重复 vs 整个响应失败
# 配套：上层 stream consumer（Streamlit）需要处理"重复内容"
```

**为什么不能缓存已 yield 内容？** 因为 stream 可能是 thinking_delta + text_delta 混合，且包含 partial 工具参数，缓存所有 chunk 会大量占内存。

#### A8. 写文档时只给"教训"不讲"反模式"

**症状**：文档堆满"已删除 / 已过期 / deprecated"标注 → 新人误以为这些功能还可能恢复 → 误用。

**反模式**：
```markdown
# ❌ 反模式：保留过期章节
"§X.X Forked Agent 不做（GLM 无 cache）"
```

**正确做法**（P1 修复后）：
```markdown
# ✅ 直接删除旧章节，不保留任何"已过期"标注
# 如果某功能已不再适用：删除整节，不留尾巴
# 如果某决策被推翻：用新决策覆盖旧决策，不留时间线
```
...
| 测试文件 | 测试数 | 覆盖范围 |
|---------|--------|---------|
| `test_context.py` | ~87 | CompactOrchestrator / ContextBudgetManager / TokenCounter / PreservedHead / UnifiedCompactPrompt |
| `test_session.py` | ~57 | SessionStorage / SessionManager / 持久化 / 恢复 / Boundary |
| `test_usage_baseline_restore.py` | 8 | Usage 持久化 / Baseline 恢复 / O(1) tail / Fallback / 压缩失效 |
...
| 文件 | 行数 | 功能 |
|------|------|------|
| `agent_core/context/budget.py` | ~400 | 上下文预算管理（含 baseline 持久化恢复） |
| `agent_core/context/compact.py` | 1029 | 压缩编排器（含 Preserved Head v4） |
| `agent_core/context/manager.py` | 123 | 统一入口 |
| `agent_core/context/tokenizer.py` | 195 | Token 估算（含 tiktoken + 5 类启发式回退） |
| `agent_core/context/test_context.py` | 1304 | 单元测试 |
| `tests/test_usage_baseline_restore.py` | 258 | Baseline 持久化测试 |
| `agent_core/agent_core.py` | 841 | Agent 主体（含 `_persist_compacted_messages` + `_restore_usage_baseline`） |
| `agent_core/session/storage.py` | 852 | JSONL 持久化 |
...
### 2026-06-18（Baseline 持久化 + Preserved Head v4 + UI 修复）

```
d1c1dbd feat: usage baseline 持久化 + O(1)恢复 + UI bug 修复
b74c632 feat(compact): preserved head v4 — 硬 stop + 第一个 turn 必含
f1f690d feat(compact): preserved head v3 — 软 budget + 硬 max_turns
efef86f feat(compact): preserved head v2 — drop-not-truncate
1bc7bbc feat(compact): preserved head v1 — 配对 + budget + 截断
bc86c89 feat(compact): Fork/旧版压缩提示词统一 + router.py tool_choice 透传
a16d9bf feat(ui): sidebar 显示 cache 命中率
71b2a22 fix(usage): UsageStats 加 cached_tokens + cache_hit_rate
71babbd refactor: Config 单例 + 常量→.env 部分
```
...
| v1.6 | 2026-06-18 | **Fork 缓存命中率实测**：新增 §4.5.8 A/B 测试结果（Fork 模式 100% 缓存命中 vs 原模式 7%），验证 `tool_choice="none"` 不影响缓存，确认最小缓存前缀约需 ≥1024 tokens |
| v1.6 | 2026-06-18 | **Preserved Head v4 + Baseline 持久化**：§10.2 重写为 v1→v4 演进史 + 硬 stop 算法；新增 §3.6 Usage Baseline 持久化与恢复（jsonl entry 存储 + O(1) read_tail 恢复 + `len-1` 语义）；新增 §11.11 F5 刷新 token 跳变教训、§11.12 UI 变量命名冲突教训；更新测试统计至 152/152；追加 2026-06-18 commit 历史 |
