...
#### cache.json sidecar 已删除

原 Day 6 P3 实现的 sidecar `cache.json` 文件（存会话级 cache 命中率统计）已于 2026-06-17 全部删除（-226 行），原因是增加复杂度但价值有限，baseline 持久化已覆盖核心需求。
...
def _build_preserved_head(
    ...
    for turn in reversed(turns):
        ...
        # 硬 stop：已有选中 turn 且加上这条会超 budget → 停止
...
- **第一个 turn 总被包含**（`selected_turns` 为空时跳过 budget 检查）
- **硬 stop**：一旦超 budget 立即 break，不再尝试后续更小的 turn
...
| `agent_core/context/compact.py` | ~700 | 压缩编排器（含 Preserved Head v4） |
| `agent_core/context/manager.py` | 123 | 统一入口 |
| `agent_core/context/tokenizer.py` | 103 | Token 估算 |
| `agent_core/context/test_context.py` | 870+ | 单元测试 |
| `tests/test_usage_baseline_restore.py` | 200+ | Baseline 持久化测试 |
| `agent_core/agent_core.py` | ~700 | Agent 主体（含 `_persist_compacted_messages` + `_restore_usage_baseline`） |
