...
选项 A 重构 (2026-07-06):handler 用 ctx.append_system 累加 skills 段到 ctx.run_state.system_prompt
(替代原 stage_inputs merge)。范式:真实 TurnContext + mock agent → handle → 断言 ctx.run_state.system_prompt。
...
def _make_ctx(system_prompt: str = "BASE") -> TurnContext:
    ...
    ctx.run_state.system_prompt = system_prompt
    return ctx
...
class TestInjection:
    def test_appends_skills_section_to_system_prompt(self, tmp_path):
        ...
        assert "## Skills (mandatory)" in ctx.run_state.system_prompt
        assert "BASE" in ctx.run_state.system_prompt  # base 保留
        assert "<name>hello</name>" in ctx.run_state.system_prompt
    ...
    def test_preserves_existing_memory_block(self, tmp_path):
        ...
        assert "[记忆库" in ctx.run_state.system_prompt  # memory 保留
        assert "## Skills" in ctx.run_state.system_prompt  # skills 追加(非覆盖)

    def test_idempotent(self, tmp_path):
        # 重复 handle 不翻倍(幂等自检:section 已在 ctx.run_state.system_prompt 则跳过)
        reg = _make_registry_with_workspace(tmp_path)
        agent = _make_agent(registry=reg)
        ctx = _make_ctx("BASE")
        SkillsPromptHandler(agent).handle(ctx)
        SkillsPromptHandler(agent).handle(ctx)  # 再调一次
        assert ctx.run_state.system_prompt.count("## Skills (mandatory)") == 1  # 没翻倍
...
class TestC2Guard:
    ...
    def test_skip_when_read_not_in_tools(self, tmp_path):
        ...
        ctx = _make_ctx("BASE")
        SkillsPromptHandler(agent).handle(ctx)
        assert "## Skills" not in ctx.run_state.system_prompt
        assert ctx.run_state.system_prompt == "BASE"  # 未改
...
class TestSkipConditions:
    ...
    def test_skip_when_empty_prompt(self, tmp_path):
        ...
        ctx = _make_ctx("BASE")
        SkillsPromptHandler(agent).handle(ctx)
        assert ctx.run_state.system_prompt == "BASE"  # 无 skill → 不注入
    ...
    def test_injects_into_empty_system_prompt(self, tmp_path):
        ...
        ctx = _make_ctx("")
        SkillsPromptHandler(agent).handle(ctx)
        assert "## Skills" in ctx.run_state.system_prompt  # skills 段注入


class TestSnapshotFailureIsolation:
    def test_handler_does_not_raise_on_snapshot_error(self):
        # registry.snapshot() 抛 → handler 吞掉,不改 ctx.run_state.system_prompt
        bad_reg = MagicMock()
        ...
        result = SkillsPromptHandler(agent).handle(ctx)
        assert isinstance(result, HandlerResult)
        assert ctx.run_state.system_prompt == "BASE"  # 失败 → 不改
...
class TestSkillsConfigExposure:
    ...
    def test_skills_config_none_handler_no_raise(self):
        ...
        result = SkillsPromptHandler(agent).handle(ctx)
        assert isinstance(result, HandlerResult)
        assert ctx.run_state.system_prompt == "BASE"
