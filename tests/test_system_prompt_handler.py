...
选项 A 重构 (2026-07-06):handler 把 agent.system_prompt append 到 ctx.system_prompt
...
class TestSystemPromptHandler:
    ...
    def test_appends_to_existing_system_prompt(self):
        ...
        ctx = _make_ctx()
        ctx.system_prompt = "PRE"
        SystemPromptHandler(agent).handle(ctx)
        assert ctx.system_prompt == "PRE\n\nBASE"

    def test_skips_when_empty_system_prompt(self):
        agent = _make_agent("")
        ctx = _make_ctx()
        ctx.system_prompt = "EXISTING"
        SystemPromptHandler(agent).handle(ctx)
        assert ctx.system_prompt == "EXISTING"  # 空 base 不 append
