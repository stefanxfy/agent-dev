...
from .sandbox_backends import NullBackend, SandboxBackend, SandboxRuntimeConfig
...
class SandboxManager:
    ...
    生命周期:
      1. __init__:_config=SandboxConfig(),_backends=[],_backend=None
    ...
    不再强制单例(旧 __new__ 已去),但模块级 sandbox_manager 仍作为默认实例
    (组合根用它;非 web 入口兜底)。多 session 隔离留作独立 P1 任务(设计 §2.2)。
    """

    _instance: Optional["SandboxManager"] = None

    def __new__(cls):
        # 模块级单例(对齐 CC + 设计 §10"保留形态"):SandboxManager() 始终返同一实例,
        # 让测试 fixture 的 mock 与 production 读模块级 sandbox_manager 一致。
        # 多 session 隔离留作独立 P1 任务(本次 scope 已排除)。
    ...
    def cleanup_after_command(self) -> None:
        ...
        """
        from .sandbox_backends._cleanup import run_default_cleanup
