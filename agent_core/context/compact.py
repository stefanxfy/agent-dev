...
严格要求（验证脚本会检查）：
- 必须使用 XML 标签包裹输出
- <analysis>...</analysis>：包裹自由分析内容
- <summary>...</summary>：包裹结构化摘要
- 标签必须成对出现（开标签 + 闭标签）
- 如果不严格使用 XML 标签，验证脚本会 FAIL
...
<summary>
1. 用户目标：用户依次要求并行执行三项任务（数学计算+搜索）、获取历史知识（秦始皇）、以及多次重复指定文字段落（LangChain介绍 1次×50、《活着》结尾共7次×50/40/50/10/10/10/5）。最后发送了问候和自我介绍。
2. 关键决策：首次请求即要求"一次性调用三个工具，不要逐个调用"，助手按要求并行执行；用户多次反复要求重复同一段《活着》文字，重复次数从 50 逐步降到 5。
3. 当前状态：所有计算、搜索、文字重复任务均已完成。最后三条用户消息（"你好"、"我是小白"、"我是小黑"）尚未获得助手回复。
4. 待办事项：回应用户最新的自我介绍消息"我是小黑"（以及可能的"我是小白"）。无其他未完成的显式任务请求。
</summary>
</example>
"""
